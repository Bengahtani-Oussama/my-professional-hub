...
import type { Experience, Settings } from "@/lib/types";
...
function ExperienceDetailPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([experienceApi.get(id), settingsApi.get()])
      .then(([it, st]) => {
        if (cancelled) return;
        setItem(it);
        setSettings(st);
  ...
  return <DetailView item={item} eyebrow={t("nav.experience")} visitors={settings.visitorCount} />;
}